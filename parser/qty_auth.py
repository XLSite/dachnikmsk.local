...
with connection.cursor() as cursor:
   # query = 'UPDATE oc_product SET stock_status_id = 5, quantity = 0 WHERE upc LIKE "%tbm.ru%"'
    #query = 'UPDATE oc_product SET stock_status_id = 5, quantity = 0 WHERE product_id IN (3006890, 3009138, 3007444, 3009260)'
   # cursor.execute(query)
   # query = 'SELECT product_id, quantity, stock_status_id, upc, minimum FROM oc_product  WHERE upc LIKE "%tbm.ru%" '
    query = 'SELECT product_id, quantity, stock_status_id, upc, minimum FROM oc_product  WHERE product_id IN (3006890, 3009138, 3007444, 3009260) '
    cursor.execute(query)
    for row in cursor:
...
def thread_function(product_id):
    ...
    try:
        ...
        availability = soup.select_one('.cart-avalible__green')
        bltxt = availability.text
        tflag = "наличии"
        availability_s = availability.span.string
        print(availability_s)
